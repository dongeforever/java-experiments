...
public class DefaultMmapFile extends ReferenceResource implements MmapFile {
    ...
    public void madvise(boolean random) {
        final long beginTime = System.currentTimeMillis();
        final long address = ((DirectBuffer) (this.mappedByteBuffer)).address();
        Pointer pointer = new Pointer(address);
        int ret;
        if (random) {
            ret = LibC.INSTANCE.madvise(pointer, new NativeLong(this.fileSize), LibC.MADV_RANDOM);
        } else {
            ret = LibC.INSTANCE.madvise(pointer, new NativeLong(this.fileSize), LibC.MADV_NORMAL);
        }
        logger.info("madvise random={} addr={} name={} size={} ret={} time consuming={}", random, address, this.fileName, this.fileSize, ret, System.currentTimeMillis() - beginTime);
